...
// Selection
idCVar aa_targetMaxDistance							(	"aa_targetMaxDistance",							"5000",		CVAR_FLOAT, "The Maximum Distance away for a target to be considered for adhesion, friction and target lock-to" );
idCVar aa_targetSelectionRadius						(	"aa_targetSelectionRadius",						"256.0",	CVAR_FLOAT, "Radius used to select targets for auto aiming" );

// Adhesion
idCVar aa_targetAdhesionRadius						(	"aa_targetAdhesionRadius",						"128.0",		CVAR_FLOAT, "Radius used to apply adhesion amount" );
idCVar aa_targetAdhesionYawSpeedMax					(	"aa_targetAdhesionYawSpeedMax",					"0.8",		CVAR_FLOAT, "Max Yaw Adhesion Speed" );
idCVar aa_targetAdhesionPitchSpeedMax				(	"aa_targetAdhesionPitchSpeedMax",				"0.8",		CVAR_FLOAT, "Max Pitch Adhesion Speed" );
idCVar aa_targetAdhesionContributionPctMax			(	"aa_targetAdhesionContributionPctMax",			"0.5",		CVAR_FLOAT, "Max Adhesion Contribution Percentage - Range 0.0 - 1.0" );
idCVar aa_targetAdhesionPlayerSpeedThreshold		(	"aa_targetAdhesionPlayerSpeedThreshold",		"0.0",		CVAR_FLOAT, "Speed Threshold that determines how fast the player needs to move before adhesion is allowed to kick in" );
...
void idAimAssist::UpdateNewAimAssist() {
	...
	angleCorrection = ang_zero;
	frictionScalar = 1.0f;
	idEntity* lastTarget = targetEntity.GetEntity();
	targetEntity = NULL;
...
idEntity* idAimAssist::FindAimAssistTarget( idVec3& targetPos ) {
	...
	idEntity * entity = gameLocal.aimAssistEntities.Next();
	...
	for ( idEntity * entity = gameLocal.aimAssistEntities.Next(); entity != NULL; entity = entity->aimAssistNode.Next() ) {
		...
		if ( entity->IsType( idActor::Type ) ) {
			...
			if ( actor->team == player->team ) {
				...
				if ( gameLocal.gameType == GAME_TDM || gameLocal.gameType == GAME_SP ) {
					continue;
				}
			}
		}
